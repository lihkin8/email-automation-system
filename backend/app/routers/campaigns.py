...
async def run_follow_ups(
    ...
    Delegates to the shared scheduler_service so that the automated daily
    scheduler and this endpoint use identical logic.
    """
    from app.services.scheduler_service import run_follow_ups_for_campaign

    result = await run_follow_ups_for_campaign(
        campaign_id=campaign_id,
        session=session,
        delay_seconds=delay_seconds,
    )

    # Surface skip conditions as HTTP errors so existing callers get the
    # same HTTP semantics they received before the refactor.
    if result.get("skipped"):
        reason = result["skipped"]
        if reason == "campaign not found":
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Campaign not found")
        if reason == "no follow-up template":
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Follow-up template not configured",
            )
        if reason == "follow-up template not found":
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Template not found")
        if reason == "no Gmail token":
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Gmail is not connected. Please connect Gmail in Settings before sending follow-ups.",
            )
        # Fallback for any other skip reason.
